"""Render STL or 3MF files to PNG previews (orthographic and perspective).

Render functions are copied verbatim from tools/library-builder/stl_to_png.py
to avoid sys.path fragility in Docker. 3MF files are converted to a numpy-stl
Mesh in-memory before being passed to the render functions.
"""
...
def _load_mesh(file_path: str):
    """Load a mesh from an STL or 3MF file and return a numpy-stl Mesh object."""
    ext = Path(file_path).suffix.lower()
    if ext == '.stl':
        return mesh.Mesh.from_file(file_path)
    elif ext == '.3mf':
        if trimesh is None:
            raise ImportError("trimesh is required to load 3MF files. Install with: pip install trimesh")
        scene = trimesh.load(file_path, force='scene')
        if hasattr(scene, 'dump'):
            combined = scene.dump(concatenate=True)
        else:
            combined = scene
        # Build a numpy-stl Mesh from trimesh vertices/faces
...
# ---------------------------------------------------------------------------
# Copied verbatim from tools/library-builder/stl_to_png.py
# ---------------------------------------------------------------------------
...
def render_stl_to_png(stl_path, output_path, max_dimension=800, dpi=100, quiet=False, rotation=0):
    ...
    Args:
        stl_path: Path to input STL or 3MF file
    ...
    """
    try:
        # Load the STL file (supports STL and 3MF)
        stl_mesh = _load_mesh(stl_path)
...
def render_all(file_path: str, output_dir: str, item_id: str) -> dict:
    ...
    ok = render_stl_to_png(file_path, str(out / ortho_filename), max_dimension=800, quiet=True)
    if not ok:
        raise RuntimeError(f"Orthographic render failed for {file_path}")
    persp_filenames = []
    for deg in [0, 90, 180, 270]:
        filename = f"{item_id}-p{deg}.png"
        ok = render_stl_to_png_perspective(file_path, str(out / filename), max_dimension=800, rotation=deg, quiet=True)
        if not ok:
            raise RuntimeError(f"Perspective render failed at {deg}° for {file_path}")
        persp_filenames.append(filename)
    return {"imageUrl": ortho_filename, "perspImageUrls": persp_filenames}
